...
use StartMeUp\Http\Requests\StoreLocalityRequest;
...
class LocalitiesController extends Controller
{
    ...
    public function index()
    {
        ...
        // Laravel Eloquent
        list($localities, $paginator) = $this->_getCollectionAndPaginator(new LocalitiesRepository());

        // Fractal
        $resource = new FractalResource\Collection($localities, new GenericTransformer());
    ...
    public function store(Request $request)
    {
        ...
        $localityData = $this->_getRequestData($request, 'locality');
        $rules = $this->_getValidationRules(new StoreLocalityRequest());

        $validator = Validator::make($localityData, $rules);
        if ($validator->fails()) {
            return response()
        ...
        $locality = new Locality($localityData);
        if ($locality->save()) {
            $resource = new FractalResource\Item($locality, new IdOnlyTransformer());
    ...
    {
        // Laravel Eloquent
        $localitiesRepository = new LocalitiesRepository();
        $locality = $localitiesRepository->find($id);
        if (!$locality) {
            return response()
                ->json($this->_getResponseDataMessageEntityDoesNotExistError('Locality', $id))
                ->setStatusCode(Response::HTTP_NOT_FOUND);
        }

        // Fractal
        $resource = new FractalResource\Item($locality, new GenericTransformer());
